...
sequenceDiagram
    ...
    Note over ID,AT: 检查触摸事件ANR
    ID->>AT: firstTimeout()
    AT-->>ID: 返回最早超时时间
    
    alt 当前时间 >= 超时时间
        ID->>AT: firstToken()
        AT-->>ID: 返回超时的IBinder token
        ID->>ID: getConnectionLocked(token)
        ID-->>Conn: 获取Connection对象
        ID->>Conn: 设置 responsive = false
        ID->>AT: eraseToken(token)
        ID->>ID: onAnrLocked(connection)
        
        Note over ID,Conn: 执行ANR处理
        ID->>Conn: 检查 waitQueue.empty()
        alt waitQueue不为空
            ID->>Conn: 获取 oldestEntry
            ID->>ID: 计算等待时间 currentWait
            ID->>ID: 生成reason字符串
            ID->>ID: updateLastAnrStateLocked()
            ID->>ID: postCommandLocked(CommandEntry)
        end
    end
    
    Note over ID,NIM: 异步执行ANR通知
    ID->>ID: doNotifyAnrLockedInterruptible()
    ID->>NIM: notifyAnr(inputApplicationHandle, token, reason)
    NIM->>IMS: notifyANR() [JNI调用]
    IMS->>IMC: notifyANR()
    
    Note over IMC,WMS: WindowManager处理
    IMC->>IMC: notifyANRInner()
    IMC->>WMS: 查找windowState和activity
    IMC->>WMS: saveANRStateLocked()
    IMC->>AMS: inputDispatchingTimedOut() [通过mAmInternal]
    
    Note over AMS,AH: ActivityManager处理
    AMS->>AMS: inputDispatchingTimedOut()
    AMS->>AH: appNotResponding()
    AH->>AH: 添加AnrRecord到mAnrRecords
    AH->>AH: startAnrConsumerIfNeeded()
    AH->>ACT: start() [创建AnrConsumerThread]
    
    Note over ACT,PR: 异步处理ANR记录
    ACT->>ACT: run()
    loop 处理ANR队列
        ACT->>AH: next() 获取AnrRecord
        ACT->>PR: appNotResponding()
        PR->>PR: 检查进程状态(shutdown/crash/killed等)
        alt 进程正常
            PR->>PR: setNotResponding(true)
            PR->>PR: 收集firstPids和lastPids
            PR->>PR: 生成堆栈跟踪文件
            PR->>PR: 记录CPU信息
            PR->>PR: 发送SIGNAL_QUIT信号
            alt 非后台ANR且非调试状态
                PR->>AMS: 发送SHOW_NOT_RESPONDING_UI_MSG
                Note right of AMS: 显示ANR对话框
            else 后台ANR且非调试状态
                PR->>PR: kill("bg anr")
            end
        end
