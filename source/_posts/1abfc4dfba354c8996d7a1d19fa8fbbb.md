...
```xml
全局（/dev/binder 的 context）
┌──────────────────────────────┐
│ binder_context               │
│   binder_context_mgr_node ─────────┐ (指向 SM 进程里的 binder_node)
└──────────────────────────────┘     │
									 │						
ServiceManager 进程                   				 
┌──────────────────────────────┐
│ binder_proc(SM)              │
│  nodes:                      │
│    [binder_node (SM 自身)] 	   ◄─┘  (登记为 context_mgr_node)
│  refs_by_desc:
│    [对各系统服务的 binder_ref (desc=k...)] → 对应服务进程的 binder_node
└──────────────────────────────┘
...
│  nodes: (可能为空或本地对象)  │
│  refs_by_desc:               │
│    handle= N → binder_ref → (系统服务 binder_node)
│                              │
│  (注意：无 handle=0 的 binder_ref；0 是直通上下文管理者) 
└──────────────────────────────┘

事务路径（获取服务）:
BpBinder(0) --transact--> (驱动特判0) --> binder_context_mgr_node --> 唤醒SM
SM 查表返回服务 flat_binder_object
驱动为 A 创建 binder_ref(handle=N)
A 构造 BpBinder(N)
```
